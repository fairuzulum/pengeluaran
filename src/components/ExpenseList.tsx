...
export const ExpenseList: React.FC<ExpenseListProps> = ({
  ...
  const paidCount = activeExpenses.filter(
    (e) => e.isPaidThisMonth
  ).length;
  ...
  const startEdit = (item: ExpenseItem) => {
    ...
    setFormDueDate(
      item.dueDateDay ? item.dueDateDay.toString() : ''
    );
  ...
  const handleSaveEdit = (id: string) => {
    ...
    const amountNum =
      parseFloat(formAmount.replace(/\D/g, '')) || 0;

    const dueDayNum = formDueDate
      ? parseInt(formDueDate, 10)
      : undefined;
  ...
  const handleSaveNew = (e: React.FormEvent) => {
    ...
    onAddExpense({
      ...
      // Expense baru mulai dihitung dari bulan sekarang
    ...
    >
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-700 mt-0.5">
            Daftar kewajiban cicilan dan kebutuhan rutin bulanan
            yang harus dipenuhi
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-700 font-medium">
              {paidCount} dari {activeExpenses.length} item (
              {paidPercentage}%)
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 font-medium">
            ...
            <span className="text-rose-700">
              Sisa Tagihan: {formatRupiah(remainingToPay)}
            </span>
          </div>
        </div>
      ...
      {isAddingNew && (
        <form
          ...
          className="p-4 sm:p-5 bg-indigo-50/50 border-b border-indigo-100 animate-in fade-in"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <div>
              ...
              <select
                value={formEndMonth}
                onChange={(e) =>
                  setFormEndMonth(e.target.value)
                }
                className="w-full text-xs px-3 py-2 border rounded-lg bg-white border-slate-300 focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
              >
                <option value="">
                  Tidak ada / berlangsung terus
                </option>
              ...
              <p className="text-[10px] text-slate-700 mt-1">
                Khusus cicilan: pilih bulan terakhir tagihan
                ini habis/lunas
              </p>
            </div>
          </div>
      ...
      <div className="divide-y divide-slate-100">
        {filteredExpenses.map((item) => {
          ...
          const meta =
            ...
            CATEGORY_META.lainnya;

          const isFinished =
            Boolean(item.endMonthYear) &&
            !isActiveThisMonth(item);

          if (isEditing) {
            return (
              <div
                key={item.id}
                className="p-4 bg-amber-50/50 border-l-4 border-amber-500"
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">
                    ...
                    {item.endMonthYear &&
                      (() => {
                        const monthOpt =
                          monthOptions.find(
                            (o) =>
                              o.value === item.endMonthYear
                          );

                        const label = monthOpt
                          ? monthOpt.label
                          : item.endMonthYear;

                        const remaining = monthDiff(
                          currentMonthYear,
                          item.endMonthYear
                        );
